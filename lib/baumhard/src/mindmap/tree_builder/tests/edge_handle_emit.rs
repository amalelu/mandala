...
fn test_edge_handles_straight_edge_emits_midpoint() {
    ...
    let kinds: Vec<&EdgeHandleKind> = scene.edge_handles.iter().map(|h| &h.kind).collect();
...
fn test_edge_handles_curved_edge_emits_control_points_not_midpoint() {
    ...
    let edge_idx = map.edges.iter().position(|e| e.visible).unwrap();
    map.edges[edge_idx]
        .control_points
    ...
    assert!(scene
        .edge_handles
        .iter()
        .any(|h| matches!(h.kind, EdgeHandleKind::ControlPoint(0))));
    assert!(scene
        .edge_handles
        .iter()
        .all(|h| !matches!(h.kind, EdgeHandleKind::Midpoint)));
}
...
fn test_edge_handles_cubic_edge_emits_both_control_points() {
    ...
    map.edges[edge_idx]
        .control_points
        .push(crate::mindmap::model::ControlPoint { x: 10.0, y: 10.0 });
    map.edges[edge_idx]
        .control_points
    ...
    assert!(scene
        .edge_handles
        .iter()
        .any(|h| matches!(h.kind, EdgeHandleKind::ControlPoint(1))));
}
