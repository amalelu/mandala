...
//! target-tree elements by their [`BranchChannel`] and dispatches
//! each mutator's effect. Handles straight-line alignment,
//! `Instruction::RepeatWhile` loop expansion (with the
//! [`Predicate`] language), and the event-propagation side channel
//! that `GlyphTreeEvent`s ride on. The single entry point is
//! [`walk_tree_from`]; everything else in this module is internal
//! scaffolding kept `pub` so mutator-DSL authors can compose
//! custom terminators without forking the walker.
