...
mod tests {
    ...
    /// mutator for state B, must end at the same observable state as
    /// a fresh `picker_glyph_areas(B)`. Pins the contract that the
    /// dynamic phase composes correctly on top of a layout-built
    /// tree — without it the per-frame perf path could silently leave
    /// stale color or scale values on cells the dynamic spec doesn't
    /// list.
    ///
    /// Why dynamic-after-layout (not just dynamic alone): the dynamic
    /// spec deliberately omits position / bounds / line_height /
    /// outline. A dynamic-only apply onto a state-A tree would leave
    /// those four fields at A's values — correct for hover/HSV (those
    /// fields don't change) but incorrect across an explicit layout
    /// move. The layout phase is what makes the dynamic phase safe.
    #[test]
    fn picker_dynamic_mutator_composes_on_layout_built_tree() {
        ...
        for ((c_got, a_got), (c_exp, a_exp)) in got.iter().zip(expected.iter()) {
            ...
            // Static fields come from the layout phase — verify
            // they're still the layout-correct values.
            assert_eq!(a_got.position, a_exp.position, "position on ch {c_got}");
            assert_eq!(
                a_got.render_bounds, a_exp.render_bounds,
                "bounds on ch {c_got}"
