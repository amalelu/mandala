...
- **Public items in baumhard carry `///` doc comments.** Every `pub`
  function, type, and module under `lib/baumhard/src/` gets a doc
  comment that explains *what it does, why it exists, and what it
  costs*. "Costs" is specific to baumhard — note an O(n) walk, an
  allocation, a clone, a lock acquisition. See
  [`lib/baumhard/CONVENTIONS.md §B8`](./lib/baumhard/CONVENTIONS.md).
  `cargo doc -p baumhard --no-deps` is a first-class deliverable.
