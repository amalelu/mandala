# THIS DOCUMENT IS NOW DEPRECATED, EVEN IF ALL FEATURES HAVE NOT BEEN IMPLEMENTED YET. IT EXISTS FOR REFERENCE.
...
- **Console rebuild** — the `/`-triggered CLI is rebuilt around a trait-dispatched cross-cutting command layer, a live prefix-match completion popup, and a correctly-measured rounded-border frame. Generic commands (`color bg=accent text=#fff border=reset`, `font size=14`, `label text="hi" position=middle`) dispatch through capability traits (`HasBgColor` / `HasTextColor` / `HasBorderColor` / `HasFontSize` / `HasLabel`) implemented on a closed `TargetView { Node \| Edge \| Portal }` enum, fanning out over `SelectionState::Multi` so shift-clicked nodes all update at once. Component-specific commands (`anchor from=top to=auto`, `body glyph=dash`, `cap from=arrow to=none`, `spacing value=wide`, `edge type=cross_link` / `edge reset=straight`, `portal create` / `portal delete` / `portal glyph=star color=accent`) stay in their own files. Parser keeps the shell-style tokenizer (quotes + escapes) but gains first-class `key=value` tokens via `Args::kvs()` / `Args::kv(name)`; all `--flag` machinery is gone. Completion is redesigned around a `CompletionContext { CommandName \| Token { index } \| KvValue { key } }` enum, prefix-matched only (no fuzzy), live-recomputed on every keystroke, and navigated with `↑`/`↓` in the popup vs. history when the popup is empty. `Tab` accepts the highlighted row; `Esc` clears the popup first, closes the console second. Visuals use baumhard's `BorderGlyphSet::box_drawing_rounded` drawn as four exactly-sized buffers with display-width-measured cols, the loud cyan swapped for a muted slate-teal palette in `console::visuals`, accent `❯`, a per-line-kind gutter glyph (`▏`) in accent/error color, and scrollback rows that fade by row age. Wide-char scrollback (`日本語`) clips cleanly through the new `baumhard::util::grapheme_chad::truncate_to_display_width` — the right border stays put. Dropped: `alias`, `select`, `mutate`, `connection` (folded into `edge`), the `BACKCOMPAT_INVOCATIONS` table, fuzzy scoring, flag parsing, and the user-mutation / bindings-overlay persistence files (deferred to a follow-up). Adds two new baumhard primitives (`grapheme_display_width`, `BorderGlyphSet::side_border`) and a new `UndoAction::EditNodeStyle` variant so the node leg of the trait setters has proper undo coverage. 656 tests passing.
- **Color picker v2 — widget feel, transparent backdrop, halos, RMB-resize** — Iterates the glyph-wheel picker into a true floating widget. The old `font_size = min(22, h/12)` formula was effectively constant on any window taller than 264 px, so the picker filled small viewports; the new canonical sizing inverts the geometry chain — `target_side = min(screen_w, screen_h) * target_frac * size_scale`, `font_size = clamp(target_side / wheel_side_in_fonts, font_min, font_max)` with a final safety clamp against tiny viewports. `wheel_side_in_fonts` is computed from the dimensionless `cell_advance / measurement_font_size` and `ring_advance / (measurement_font_size * ring_scale)` ratios captured at picker open, so the wheel stays proportionally correct across every script's per-glyph shape. New `target_frac` (0.38), `font_min`/`font_max` (7/28), `outline_px`/`outline_samples` (3 / 8), `transparent_backdrop` (true), `resize_scale_min`/`max` (0.4 / 2.0) live in `color_picker.json`. The opaque backdrop rect is gated by `transparent_backdrop` and now defaults off; `build_color_picker_overlay_tree` emits N halo `GlyphArea` copies (8 evenly-spaced offsets at the scaled `outline_px`) before each colored glyph so the picker stays legible against any canvas content. Right-mouse-button drag inside the existing `DragAnchor` region runs a multiplicative resize gesture: `new_scale = anchor_scale * (cursor_radius / anchor_radius)` clamped to `[resize_scale_min, resize_scale_max]`, with `anchor_radius` floored at `font_size * 3.0` to prevent sensitivity explosions when grabbing near the wheel center. The old `drag: Option<DragState>` field is replaced by `gesture: Option<PickerGesture>` (Move + Resize variants), making mutual exclusion a type invariant. **Egyptian hieroglyph fix**: the tree walker (`walk_tree_into_buffers` in `renderer.rs`) had a long-standing bug where it stored `region.font` on the `GlyphArea` but never threaded it into the cosmic-text `Attrs::family()`, so the bottom-arm hieroglyphs silently rendered as tofu. The walker now pre-computes per-region family names from `COMPILED_FONT_ID_MAP` and pins them via `Family::Name` so the explicit font hint actually reaches shaping. New tests: `layout_targets_screen_short_side_fraction`, `layout_scales_with_user_size_scale`, `layout_font_shrinks_on_small_windows`, `layout_uses_dimensionless_advance_ratios`, `layout_safety_clamp_dominates_user_scale_on_tiny_screens`, `resize_gesture_scale_math_is_multiplicative`. **Performance note**: 8-sample halos multiply per-glyph cosmic-text shaping by 9× — acceptable on the picker's modal frame, but the existing hover-rebuild regression (re-shape on every cursor move) compounds this. The mutator-tree follow-up tracked at `Color-picker hover regression` subsumes both: after that lands, halos become static tree nodes touched only on font_size changes, and the hover path mutates only changed cell colors.

- **656 tests passing** — run via `./test.sh`; see `TEST_CONVENTIONS.md` for the testing philosophy and `./test.sh --coverage` for `cargo-llvm-cov` reports

- **Unified Baumhard rendering pipeline** — every visual component now renders through baumhard `Tree<GfxElement, GfxMutator>` instances composited via a top-level `Scene` (`lib/baumhard/src/gfx_structs/scene.rs`) and an app-layer `AppScene` host (`src/application/scene_host.rs`). Borders, connections, portals, connection labels, edge handles ride a *canvas-space* sub-scene (camera-transformed); the console and color picker ride a *screen-space* sub-scene. Each component has a `build_*_tree` builder under `lib/baumhard/src/mindmap/tree_builder.rs` (or `src/application/{console,color_picker}/`), and a single `Renderer::rebuild_canvas_scene_buffers` / `rebuild_overlay_scene_buffers` walker drives glyph shaping. The legacy bespoke buffer rebuild paths (`rebuild_border_buffers*`, `rebuild_connection_buffers*`, `rebuild_portal_buffers`, `rebuild_edge_handle_buffers`, `rebuild_connection_label_buffers`, the static/dynamic color-picker split) are deprecated; legacy console + picker `_legacy` wrappers are gone, dead `Renderer::load_mindmap` and `RenderDecree::LoadMindMap` removed. Hit-testing now has both `Tree::descendant_at` (innermost AABB) and `Scene::component_at` (top-down across trees) primitives, with an O(1) memoised `descendants_aabb` per tree (cached, invalidated by `MutatorTree::apply_to`). Color-picker hover currently re-shapes the whole picker tree per frame (known regression); a follow-up will emit a `MutatorTree<GfxMutator>` that touches only changed cell colors and the indicator's position.
...
- **Color-picker hover regression** — the migration to the overlay tree path lost the legacy static / dynamic split that skipped re-shaping the 24-slot hue ring on hover. Today every cursor move inside the picker re-shapes the whole tree (~64 glyphs). The fix is a tree-mutator-only hover update per §B1; tracked as the unified-rendering follow-up.
- **Connection / border drag re-shape on every frame** — same root cause: legacy `_keyed` paths skipped re-shaping unchanged elements, the tree path doesn't yet have an incremental shaping cache. Drag perf on very dense maps is noticeably worse than pre-migration. Same fix as picker hover.
- **Mindmap nodes still on the standalone tree** — every other component lives in `AppScene`'s canvas sub-scene; only the mindmap-text tree (`MindMapDocument::build_tree` → `rebuild_buffers_from_tree`) remains separately managed in the event loop. The intended end state is to register the mindmap tree as the lowest layer in `AppScene::canvas` too, but threading it through the 30+ `rebuild_all` callers is a future micro-session.
- **Hit-test still goes through document path** — `document::hit_test`, `hit_test_edge`, etc. are still consulted directly. `Scene::component_at` and `Tree::descendant_at` are wired and tested but no event loop dispatcher routes through them yet.
