...
```
"rectangle", "rounded_rectangle", "ellipse", "circle", "diamond",
"parallelogram", "hexagon"
```

Inherited from miMind's shape catalog (plus the `"circle"` alias).
Default: `"rectangle"`.
...
is accepted as an alias for `"ellipse"` — the convenience spelling
